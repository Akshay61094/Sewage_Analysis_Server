...
def restore_and_predict():
    ...
    def process_image(image):
        ...
        store_faulty_frames(segmentation_c,segmentation_t, np.array(street_im))
    ...
    # .subclip(2,4) #NOTE: this function expects color images!!
    ...
    if(len(crack_frames)!=0):
        crack_end.append(crack_start[-1] + len(crack_frames))
    print(len(crack_end))
    if(len(crack_set)!= 0 or len(crack_frames)!=0):
        length_c = cracks_analysis.crack_length(crack_start,crack_end)
        print(length_c)
        severity = cracks_analysis.crack_severity(crack_set, crack_frames)
        print(severity)
    # if(len(root_set)!= 0 or len(root_frames)!= 0):
...
crack_frame_pix_count = []
root_frame_pix_count = []

def store_faulty_frames(segmentation_c, segmentation_t, img_frame):

    detected = False #any fault detected
    crack_detected = False #crack detected
    root_detected = False #root detected
    global fault_not_found_duration
    global frame_count
    ...
    global crack_end 
    global crack_frames
    global crack_found_duration 
    global crack_not_found_duration

    global root_set
    global root_start
    global root_end
    global root_frames 
    global root_not_found_duration
    global root_found_duration
    ...
    if detected:
        if (crack_detected):
            ...
            if crack_found_duration == 10:
                crack_start.append(frame_count)#start of fault
            if crack_found_duration >= 10:   
                crack_frames.append(img_frame)
                crack_not_found_duration = 0
        ...
        if(fault_not_found_duration == 10):
            if(len(crack_frames) != 0):
                crack_end.append(frame_count)#end of fault
                crack_detected = False
                crack_set.append(crack_frames)
                crack_frames=[]
                crack_found_duration = 0
            if(len(root_frames) != 0):
                root_end.append(frame_count)
                root_detected = False
                root_set.append(root_frames)
                root_frames=[]
                root_found_duration = 0    
    frame_count+=1 
    # return crack_set, root_set, crack_frames, root_frames
