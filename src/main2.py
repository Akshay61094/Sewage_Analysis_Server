...
def load_vgg(sess, vgg_path):
    ...
    lyr_3 = graph.get_tensor_by_name(vgg_layer3_out_tensor_name);
    lyr_4 = graph.get_tensor_by_name(vgg_layer4_out_tensor_name);
    lyr_7 = graph.get_tensor_by_name(vgg_layer7_out_tensor_name);
    return img_input, keep_prob, lyr_3, lyr_4, lyr_7;
...
def layers(vgg_layer3_out, vgg_layer4_out, vgg_layer7_out, num_classes):
    ...
    """
    # TODO: Implement function
    lyr7_conv1x1 = tf.layers.conv2d(vgg_layer7_out, num_classes, 1, 1, padding="SAME", kernel_regularizer =
                            tf.contrib.layers.l2_regularizer(1e-3));
    lyr_7_upsample = tf.layers.conv2d_transpose(lyr7_conv1x1, num_classes, 4, 2, padding = "SAME",
                            kernel_regularizer = tf.contrib.layers.l2_regularizer(1e-3));
    lyr4_conv1x1 = tf.layers.conv2d(vgg_layer4_out, num_classes, 1, 1, padding="SAME", kernel_regularizer =
                            tf.contrib.layers.l2_regularizer(1e-3));
    ...
    lyr3_conv1x1 = tf.layers.conv2d(vgg_layer3_out, num_classes, 1, 1, padding="SAME", kernel_regularizer =
                            tf.contrib.layers.l2_regularizer(1e-3));
    lyr3_lyr4_7_add = tf.add(lyr7_ly4_upsampled, lyr3_conv1x1);
    lyr3_4_7_upsampled = tf.layers.conv2d_transpose(lyr3_lyr4_7_add, num_classes, 16, 8, padding = "SAME",
                            kernel_regularizer = tf.contrib.layers.l2_regularizer(1e-3));

    return lyr3_4_7_upsampled;
...
def optimize(nn_last_layer, correct_label, learning_rate, num_classes):
    ...
    correct_prediction = tf.equal(tf.argmax(nn_last_layer, 1), tf.argmax(correct_label, 1))
    accuracy_operation = tf.reduce_mean(tf.cast(correct_prediction, tf.float32))
    return nn_last_layer, training_operation, cross_entropy_loss
...
#     tests.test_for_kitti_dataset(data_dir)
    ...
    with tf.Session() as sess:
        ...
        nn_last_layer, train_op, cross_entropy_loss = optimize(nn_last_layer,correct_label,learning_rate,num_classes);
